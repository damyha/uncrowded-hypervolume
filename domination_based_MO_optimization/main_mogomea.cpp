...
int main(int argc, char **argv) {
    ...
    hicam::elitist_archive_pt elitist_archive = std::make_shared<hicam::elitist_archive_t>(elitist_archive_size_target,
                                                                                           rng);
    ...
    elitist_archive->reduceArchiveSizeByHSS(approximation_set_size, mo_fitness_function->hypervolume_max_f0,
                                            mo_fitness_function->hypervolume_max_f1);
    ...
    if (print_verbose_overview) {
        std::cout << "Best: \n\tHV = " << std::fixed << std::setprecision(14)
                  << elitist_archive->compute2DHyperVolume(mo_fitness_function->hypervolume_max_f0,
                                                           mo_fitness_function->hypervolume_max_f1)
                  << "\n\tMO-fevals = " << gomea::number_of_evaluations << "\n\truntime = "
                  << double(clock() - gomea::timestamp_start) / CLOCKS_PER_SEC << " sec" << std::endl;
