...
hillvallea::gomea_t::gomea_t(const size_t number_of_parameters, const vec_t &lower_param_bounds,
                             const vec_t &upper_param_bounds, double init_univariate_bandwidth, int version,
                             fitness_pt fitness_function, rng_pt rng) : optimizer_t(number_of_parameters,
                                                                                    lower_param_bounds,
                                                                                    upper_param_bounds,
    ...
    linkage_model = std::make_shared<FOS_t>();  // Doing this for UHV.cpp does not matter but it might break other things?
    ...
    switch (version) {
        ...
        case 64: // static marginal product linkage model from distance matrix (if available)
            static_linkage = true;
            FOS_element_lb = (int) fitness_function->covariance_block_size; // Todo: Remove. This is the problem dimension
            FOS_element_ub = (int) fitness_function->covariance_block_size;
            learn_linkage_tree_from_distance_matrix = true;
            break;
...
hillvallea::optimizer_pt hillvallea::gomea_t::clone() const {
    ...
    gomea_pt opt = std::make_shared<gomea_t>(number_of_parameters, lower_param_bounds, upper_param_bounds,
                                             init_univariate_bandwidth, version, fitness_function, rng);
...
/**
 * Clones the optimizer without cloning the population
 * Todo: Not sure what the consequences are for pointing to the same rng object.
 * @param newPopulation The new population assigned to this optimizer
 * @return A clone of this optimizer with a specific population
 */
hillvallea::gomea_pt hillvallea::gomea_t::clone(const population_pt& newPopulation) const
{
    // Create a new optimizer
    /* Variable taken care of during initialization: dynamic_filter_large_FOS_elements, always_keep_largest_FOS_element,
     *  sample_conditionally, gradient_step, FOS_element_lb,
     *  learn_linkage_tree_from_distance_matrix, static_linkage, random_linkage
     *  distribution_multiplier_increase, distribution_multiplier_decrease, st_dev_ratio_threshold, eta_ams, eta_cov
     *  selection_fraction
     *
     *  Probably taken care of: FOS_element_ub
     * */
    gomea_pt cloneOptimizer = std::make_shared<gomea_t>(number_of_parameters, lower_param_bounds, upper_param_bounds,
                                                        init_univariate_bandwidth, version, fitness_function, rng);

    // Insert population into optimizer
    cloneOptimizer->pop = newPopulation;
    cloneOptimizer->population_size = newPopulation->size();
    cloneOptimizer->initial_population_size = initial_population_size;
    cloneOptimizer->best = best;
    cloneOptimizer->best_solution_index = best_solution_index;
    cloneOptimizer->population_initialized = true;

    // Copy data
    cloneOptimizer->sample_bayesian_factorization = sample_bayesian_factorization;
    cloneOptimizer->FOS_element_lb = FOS_element_lb;
    cloneOptimizer->FOS_element_ub = FOS_element_ub;
    cloneOptimizer->no_improvement_stretch = no_improvement_stretch;
    cloneOptimizer->linkage_model = linkage_model->clone();
    cloneOptimizer->mean_vector = mean_vector;
    cloneOptimizer->old_mean_vector = old_mean_vector;
    cloneOptimizer->mean_shift_vector = mean_shift_vector;
    cloneOptimizer->weighted_number_of_evaluations = weighted_number_of_evaluations;
    // Todo: Clone the gradient_methods variable (currently not used)

    // General optimizer data
    cloneOptimizer->active = active;
    cloneOptimizer->number_of_parameters = number_of_parameters;
    cloneOptimizer->use_boundary_repair = use_boundary_repair;
    cloneOptimizer->fitness_function = fitness_function;
    cloneOptimizer->number_of_generations = number_of_generations;
    cloneOptimizer->average_fitness_history = average_fitness_history;
    cloneOptimizer->maximum_no_improvement_stretch = maximum_no_improvement_stretch;
    cloneOptimizer->param_std_tolerance = param_std_tolerance;
    cloneOptimizer->fitness_std_tolerance = fitness_std_tolerance;
    cloneOptimizer->penalty_std_tolerance = penalty_std_tolerance;

    return cloneOptimizer;
}

// Algorithm Name
std::string hillvallea::gomea_t::name() const { return "GOMEA"; }
std::string hillvallea::gomea_t::convertUHV_GOMEALinkageModelNameToString()
{
    switch (version) {
...
void hillvallea::gomea_t::initialize_from_population(population_pt pop, size_t target_popsize) {
    ...
    if (gradient_step) {
        ...
        for (size_t i = 0; i < gradient_methods.size(); ++i) {
            ...
            gradient_methods[i] = std::make_shared<adam_t>(fitness_function, 50, lower_param_range, upper_param_range,
                                                           -1, -1, 0.0, false, 1234, false, false, "", "", gamma_weight,
...
void hillvallea::gomea_t::generation(size_t sample_size, int &external_number_of_evaluations) {
    ...
    if (!static_linkage || number_of_generations == 0) {
        ...
        linkage_model = updateLinkageTree(matrix, linkage_model, learn_linkage_tree_from_distance_matrix,
                                          random_linkage, static_linkage,
                                          number_of_generations, FOS_element_lb, new_temp_FOS_element_ub,
                                          always_keep_largest_FOS_element, number_of_parameters,
                                          sample_bayesian_factorization, rng);
    ...
    // Re-evaluate everything once in a while to get rid of rounding off errors (which occur surprisingly often)
    if (fitness_function->partial_evaluations_available &&
        fitness_function->has_round_off_errors_in_partial_evaluations && (number_of_generations + 1) % 50 == 0) {
        evaluateCompletePopulation();
    }

    // Retrieve current elite solution
    ...
    external_number_of_evaluations += round(
            weighted_number_of_evaluations - current_number_of_evaluations); // not the most elegant way
...
size_t hillvallea::gomea_t::recommended_popsize(const size_t problem_dimension) const {
    if (FOS_element_ub == 1) {
        return (size_t) std::max((double) ((size_t) ((2.0 / selection_fraction) + 1)),
                                 10.0 * pow((double) problem_dimension, 0.5));
    } else {
        return (size_t) std::max((double) ((size_t) ((2.0 / selection_fraction) + 1)),
                                 17.0 + 3.0 * pow((double) problem_dimension, 1.5));
    }
}
...
namespace hillvallea {
    /**
     * Clones this FOS object
     * @return A clone of this FOS object
     */
    FOS_pt FOS_t::clone() {
        FOS_pt newFOS = std::make_shared<FOS_t>();
        newFOS->sets.resize(this->sets.size());
        for(size_t i = 0; i < this->sets.size(); ++i)
        {
            newFOS->sets[i] = this->sets[i]->clone();
        }
    ...
    /**
     * Clones this FOS_element object
     * @return A clone of this object
     */
    FOS_element_pt FOS_element_t::clone() {

        FOS_element_pt newElement =  std::make_shared<FOS_element_t>();

        newElement->params = this->params;
        newElement->enable_regularization = this->enable_regularization;
        newElement->cholesky_factor_lower_triangle = this->cholesky_factor_lower_triangle;
        newElement->covariance_matrix = this->covariance_matrix;
        newElement->samples_drawn_from_normal = this->samples_drawn_from_normal;
        newElement->out_of_bounds_draws = this->out_of_bounds_draws;
        newElement->distribution_multiplier = this->distribution_multiplier;

        newElement->sample_conditionally = this->sample_conditionally;
        newElement->sample_bayesian_factorization = this->sample_bayesian_factorization;
        newElement->mean_shift_matrix_to_condition_on = this->mean_shift_matrix_to_condition_on;
        newElement->params_to_condition_on = this->params_to_condition_on;
        newElement->bayesian_factorization_indicator_matrix = this->bayesian_factorization_indicator_matrix;
    ...
    void getSimilarityMatrix(const matrix_t &similarity_matrix, bool invert_input_matrix, matrix_t &S_matrix,
                             int *index_order) {
        ...
        while (!done) {
            ...
            while (NN_chain_length < 3) {
                NN_chain[NN_chain_length] = determineNearestNeighbour(NN_chain[NN_chain_length - 1], S_matrix,
                                                                      mpm_number_of_indices, mpm_length, FOS_element_ub,
                                                                      number_of_parameters, random_linkage_tree,
                                                                      S_vector);
                NN_chain_length++;
            }
            ...
            while (NN_chain[NN_chain_length - 3] != NN_chain[NN_chain_length - 1]) {
                NN_chain[NN_chain_length] = determineNearestNeighbour(NN_chain[NN_chain_length - 1], S_matrix,
                                                                      ...
                                                                      number_of_parameters, random_linkage_tree,
                                                                      S_vector);
                if (((getSimilarity(NN_chain[NN_chain_length - 1], NN_chain[NN_chain_length], FOS_element_ub,
                                    number_of_parameters, mpm_number_of_indices, random_linkage_tree, S_matrix,
                                    S_vector) ==
                      getSimilarity(NN_chain[NN_chain_length - 1], NN_chain[NN_chain_length - 2], FOS_element_ub,
                                    number_of_parameters, mpm_number_of_indices, random_linkage_tree, S_matrix,
                                    S_vector)))
    ...
    void filterFOS(FOS_pt input_FOS, int lb, int ub, bool always_keep_largest_FOS_element) {
        ...
        for (size_t i = 0; i < input_FOS->length(); ++i) {
            if (input_FOS->sets[i]->length() == largest_FOS_element ||
                (input_FOS->sets[i]->length() >= lb && input_FOS->sets[i]->length() <= ub)) {
    ...
    double getSimilarity(size_t a, size_t b, int FOS_element_ub, size_t number_of_parameters,
                         const std::vector<size_t> &mpm_number_of_indices, bool random_linkage_tree,
                         const matrix_t &S_matrix, const vec_t &S_vector) {
        if (FOS_element_ub < number_of_parameters &&
    ...
    size_t
    determineNearestNeighbour(size_t index, const matrix_t &S_matrix, const std::vector<size_t> &mpm_number_of_indices,
                              int mpm_length, int FOS_element_ub, size_t number_of_parameters, bool random_linkage_tree,
                              const vec_t &S_vector) {
        int i, result;
        ...
        if (result == index)
            result++;
        for (i = 1; i < mpm_length; i++) {
            if (((getSimilarity(index, i, FOS_element_ub, number_of_parameters, mpm_number_of_indices,
                                random_linkage_tree, S_matrix, S_vector) >
                  getSimilarity(index, result, FOS_element_ub, number_of_parameters, mpm_number_of_indices,
                                random_linkage_tree, S_matrix, S_vector))
                 || ((getSimilarity(index, i, FOS_element_ub, number_of_parameters, mpm_number_of_indices,
                                    random_linkage_tree, S_matrix, S_vector) ==
                      getSimilarity(index, result, FOS_element_ub, number_of_parameters, mpm_number_of_indices,
                                    random_linkage_tree, S_matrix, S_vector)) &&
        ...
        for (i = (int) 0; i < prev_FOS->length(); i++) {
            for (j = (int) 0; j < new_FOS->length(); j++) {
                ...
                FOS_element_similarity_matrix[i][j] =
                        (int) 10000 * (2.0 * matches / (prev_FOS->sets[i]->length() + new_FOS->sets[j]->length()));
            }
        }
                ...
                while (rd < wr) {
                    ...
                    for (y = 0; y < dim; y++) {
                        if (similarity_matrix[x][y] == lx[x] + ly[y] && !T[y]) {
                            ...
                            T[y] = 1;
                            q[wr++] = yx[y];
                            hungarianAlgorithmAddToTree(yx[y], x, S, prev, slack, slackx, lx, ly, similarity_matrix,
                                                        dim);
                        }
                    }
                    if (y < dim) break;
...
namespace hillvallea {
    ...
    FOS_pt updateLinkageTree
        ...
        new_FOS = learnLinkageTree(input_matrix, invert_similarity_matrix, random_linkage, number_of_parameters, rng,
                                   temp_FOS_element_ub);
        ...
        if (sample_bayesian_factorization) {
            ...
            for (size_t i = 0; i < number_of_parameters; ++i) {
                if (variable_occurences[i] != 1) {
                    std::cout << "Linkage model is not marginal (variable " << i << " occurs " << variable_occurences[i]
                              << " times), so bayesian factorization is disabled\n";
                    sample_bayesian_factorization = false;
                    break;
                }
            }
            ...
            if (sample_bayesian_factorization) {
                ...
                full_bayesian_fos->bayesian_factorization_indicator_matrix.resize(number_of_parameters,
   ...
   */
    short gomea_t::checkFitnessVarianceTerminationSinglePopulation() {
      ...
      for(int j = 0; j < linkage_model->set_length[i]; j++ ) {
        ...
        bool terminate_on_fitness_std = (pop->average_constraint() == 0) && (pop->size() > 1) &&
                                        (pop->relative_fitness_std() < fitness_std_tolerance);
        bool terminate_on_penalty_std = (pop->average_constraint() > 0) && (pop->size() > 1) &&
                                        (pop->relative_constraint_std() < penalty_std_tolerance);
        ...
        if (terminate_for_param_std_mean_zero || terminate_on_parameter_std || terminate_on_fitness_std ||
            terminate_on_penalty_std) {
            active = false;
            return !active;
        }
...
//        printf("Dynamic filter large FOS elements: %d\n", dynamic_filter_large_FOS_elements); // Todo: Remove this
        int temp_FOS_element_ub = FOS_element_ub;
        ...
        // I don't think this is used for marginal and linkage tree
        ...
        // Apply the GOM operator
        bool full_fos_element_included = false;
        for (int oj = 0; oj < linkage_model->length(); oj++) {
            int FOS_index = fos_order[oj];

            // skip fos elements that are too small or too large. Also filters out empty FOS sets
            if (linkage_model->sets[FOS_index]->length() < FOS_element_lb ||
                linkage_model->sets[FOS_index]->length() > temp_FOS_element_ub) {
                if (!always_keep_largest_FOS_element ||
                    linkage_model->sets[FOS_index]->length() != largest_FOS_element_size) {
                    continue;
                }
            }
            ...
            population_t selection;
            makeSelection(*pop, selection_fraction,
                          selection); // TODO: this is double because we also do it just before this function call in generation(); fix later.
            pop->sols[0] = copySolution(selection.sols[0]);
            ...
            if (!linkage_model->sets[FOS_index]->enable_regularization) {
                choleskyDecomposition(linkage_model->sets[FOS_index]->covariance_matrix,
                                      linkage_model->sets[FOS_index]->cholesky_factor_lower_triangle, success);
            }

            if (!success || linkage_model->sets[FOS_index]->enable_regularization) {
                linkage_model->sets[FOS_index]->enable_regularization = true;
                regularizeCovarianceMatrix(linkage_model->sets[FOS_index]->covariance_matrix, selection.sols,
                                           mean_vector, linkage_model->sets[FOS_index]->params);
                choleskyDecomposition(linkage_model->sets[FOS_index]->covariance_matrix,
                                      linkage_model->sets[FOS_index]->cholesky_factor_lower_triangle, success);
            }
            ...
            {
                if (solution_order[k] ==
                    0) { // solution_order[k] points to the solution we want to update, and sol[0] = best, so this skips the best.
                    best_hit = 1;
                    continue; // skips best in population
                }

                if (k >= (number_of_AMS_solutions +
                          best_hit)) { // keeps the number_of_ams_solutions the same if the best is hit.
                ...
                individual_improved_this_FOS_element[solution_order[k]] = generateNewSolutionFromFOSElement(
                        linkage_model->sets[FOS_index], solution_order[k], apply_AMS, force_accept_new_solutions);
                individual_improved[solution_order[k]] |= (bool) individual_improved_this_FOS_element[solution_order[k]];
            }

            // Update Hyperparameters
            size_t number_of_improvements = generationalImprovementForFOSElement(linkage_model->sets[FOS_index],
                                                                                 &sdr[FOS_index], current_elite);
            adaptDistributionMultiplier(linkage_model->sets[FOS_index]->distribution_multiplier,
                                        (number_of_improvements > 0), sdr[FOS_index],
                                        linkage_model->sets[FOS_index]->out_of_bounds_draws,
                                        linkage_model->sets[FOS_index]->samples_drawn_from_normal);
        ...
        if (number_of_generations > 0) {
            ...
            for (size_t k = 0; k < population_size; ++k) {
                ...
                if (k < number_of_AMS_solutions + best_hit) {
                    individual_improved[solution_order[k]] |= applyAMS(pop->sols[solution_order[k]],
                                                                       weighted_number_of_evaluations, best);
                }
            }
        }
        ...
        for (size_t i = 0; i < parameter_update_counter.size(); ++i) {
            if (parameter_update_counter[i] == 0) {
                std::cout << "Warning: parameter " << i << " not updated in generation " << number_of_generations << "."
                          << std::endl;
            }
        }
        ...
        if (gradient_step) {
            ...
            for (size_t i = 0; i < population_size; ++i) {
                ...
                weighted_number_of_evaluations += gradient_methods[i]->gradientOffspring(pop->sols[i],
                                                                                         touched_parameter_idx, gammas);
                gradient_methods[i]->gamma = gammas[0];
                gradient_methods[i]->number_of_generations++;
   ...
   */
        ...
        do {
            ...
            if (times_not_in_bounds >= 100) {
                ...
                for (i = 0; i < num_indices; i++)
                    result[i] = lower_param_bounds[indices[i]] +
                                (upper_param_bounds[indices[i]] - lower_param_bounds[indices[i]]) *
                                randomRealUniform01(*rng);
            } else {
                ...
                if (FOS_element->length() == 1) {
                    ...
                    result = matrixVectorMultiplication(FOS_element->cholesky_factor_lower_triangle.toArray(), z,
    ...
    double *gomea_t::generateNewConditionalPartialSolutionFromFOSElement(FOS_element_pt FOS_element,
                                                                         const solution_t &conditional_sol) {
        ...
        for (size_t i = 0; i < FOS_element->params_to_condition_on.size(); ++i) {
            delta_contidional_params[i] = conditional_sol.param[FOS_element->params_to_condition_on[i]] -
                                          mean_vector[FOS_element->params_to_condition_on[i]];
        }
        ...
        do {
            ...
            if (times_not_in_bounds >= 100) {
                ...
                if (FOS_element->length() == 1) {
                    ...
                    result[0] =
                            z[0] * sqrt(FOS_element->covariance_matrix[0][0]) + conditional_mean_vector[0]; // updated
                } else {
                    result = matrixVectorMultiplication(FOS_element->cholesky_factor_lower_triangle.toArray(), z,
                                                        (int) num_indices, (int) num_indices);
                    for (i = 0; i < num_indices; i++)
   ...
   */
    short gomea_t::generateNewSolutionFromFOSElement(FOS_element_pt FOS_element, int individual_index, short apply_AMS,
                                                     bool force_accept_new_solutions) //, bool new_solution_from_gradient_update )
    {
        int j, m;
        size_t im;
        ...
        if (apply_AMS && (number_of_generations > 0)) {
            ...
            while ((out_of_range == 1) && (shrink_factor > 1e-10)) {
                ...
                for (m = 0; m < num_indices; m++) {
                    im = indices[m];
                    result[m] = pop->sols[individual_index]->param[im] +
                                shrink_factor * delta_AMS * FOS_element->distribution_multiplier *
                                (mean_shift_vector[im]);
        ...
        if (!fitness_function->partial_evaluations_available) {
            ...
            if (gradient_step) {
                fitness_function->partial_evaluate_with_gradients(pop->sols[individual_index], touched_indices,
                                                                  backup_sol);
            } else {
        ...
        while (alpha >= 0.01) {
            ...
            for (io = 0; io < linkage_model->length(); io++) {
                ...
                for (j = 0; j < num_touched_indices; j++) {
                    pop->sols[individual_index]->param[touched_indices[j]] =
                            alpha * pop->sols[individual_index]->param[touched_indices[j]] +
                            (1 - alpha) * donor_sol.param[touched_indices[j]];
                }
   ...
   * an improvement
   */
    void gomea_t::adaptDistributionMultiplier(double &multiplier, bool improvement, double st_dev_ratio,
                                              size_t out_of_bounds_draws, size_t samples_drawn_from_normal) const {
        if (active) {
   ...
   */
    size_t gomea_t::generationalImprovementForFOSElement(FOS_element_pt FOS_element, double *st_dev_ratio,
                                                         const solution_t &elite) const {
   ...
   */
    double gomea_t::getStDevRatioForFOSElement(const vec_t &parameters, FOS_element_pt FOS_element) const {
        ...
        if (FOS_element->length() == 1) {
            ...
            inverse = matrixLowerTriangularInverse(FOS_element->cholesky_factor_lower_triangle.toArray(),
                                                   (int) FOS_element->length());
...
namespace hillvallea {
   ...
   */
    void computeRanks(const population_t &pop, vec_t &ranks) {
        ...
        for (size_t i = 1; i < pop.size(); i++) {
            ...
            if (objective_values[sorted[i]] != objective_values[sorted[i - 1]] ||
                constraint_values[sorted[i]] != constraint_values[sorted[i - 1]])
                rank++;
    ...
    void computeRanksForInitialPopulation(const population_t &pop, int initial_population_size, vec_t &ranks) {
        ...
        int *sorted = mergeSortFitness(objective_values.toArray(), constraint_values.toArray(),
   ...
   */
    ...
    void
    makeSelection(const population_t &pop, double selection_fraction, population_t &selection, bool initial_population,
                  int initial_popsize) {
        vec_t ranks;
   ...
   * based on diversity.
   */
    void makeSelectionsUsingDiversityOnRank0(const population_t &pop, double selection_fraction, vec_t &ranks,
                                             population_t &selection) {
        ...
        for (i = 0; i < number_of_rank0_solutions; i++) {
            ...
            nn_distances[i] = pop.sols[preselection_indices[i]]->param_distance(
                    pop.sols[selection_indices[number_selected_so_far - 1]]->param);
        }
        while (number_selected_so_far < selection_size) {
            ...
            for (i = 0; i < number_of_rank0_solutions; i++) {
                ...
                value = pop.sols[preselection_indices[i]]->param_distance(
                        pop.sols[selection_indices[number_selected_so_far - 1]]->param);
    ...
    //        n
    bool regularizeCovarianceMatrix(matrix_t &covariance, const std::vector<solution_pt> &sols,
                                    const std::vector<double> &mean, const std::vector<size_t> &parameters) {
        ...
        for (size_t i = 0; i < n; ++i) {
            for (size_t j = 0; j < n; ++j) {
                ...
                for (size_t k = 0; k < sols.size(); ++k) {
                    temp = (sols[k]->param[parameters[i]] - mean[parameters[i]]) *
                           (sols[k]->param[parameters[j]] - mean[parameters[j]]);
    ...
    void estimateFullCovarianceMatrixML(matrix_t &full_covariance_matrix, const population_t &selection,
                                        const vec_t &mean_vector, double init_univariate_bandwidth) {
        if (selection.size() == 0) {
            return;
        }
    ...
    double estimateCovariance(size_t vara, size_t varb, const std::vector<solution_pt> &sols, const vec_t &mean_vector,
    ...
    void estimateCovarianceMatrix(FOS_element_pt FOS_element, const population_t &selection, const vec_t &mean_vector,
                                  double init_univariate_bandwidth) {
        ...
        if (!FOS_element->sample_bayesian_factorization && selection.sols.size() < n + 1) {
            for (size_t k = 0; k < n; k++) {
                ...
                for (size_t m = k; m < n; m++) {
                    ...
                    if (vara == varb) {
                        FOS_element->covariance_matrix[k][m] = estimateCovariance(vara, varb, selection.sols,
                                                                                  mean_vector,
                                                                                  init_univariate_bandwidth);
                    } else {
   ...
   * Set the covariance matrix of all the FOS elements in the linkage model to an identity matrix of size
   *  temp_n x temp_n. where temp_n is the number of elements in the FOS element.
   * i.e. [[1,  ..., 0],
   *       [0,1,..., 0],
   *            ...,
   *       [0,  ..., 1]]
   * @param linkage_model The linkage model containing the FOS elements (FOS_element_pt)
   */
    void initializeCovarianceMatrices(FOS_pt linkage_model) {
        for (size_t i = 0; i < linkage_model->sets.size(); ++i) {
            linkage_model->sets[i]->covariance_matrix.setIdentity(linkage_model->sets[i]->length(),
                                                                  linkage_model->sets[i]->length());
        }
    }
    ...
    void estimateMeanVectorML(const population_t &selection, vec_t &mean_vector, vec_t &old_mean_vector,
