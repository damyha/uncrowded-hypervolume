...
namespace hillvallea {
    ...
    adam_t::adam_t
        ...
        for (size_t i = 0; i < lower_param_bounds.size(); ++i) {
            if (this->lower_init_ranges[i] <
                lower_param_bounds[i]) { this->lower_init_ranges[i] = lower_param_bounds[i]; }
            if (this->upper_init_ranges[i] >
                upper_param_bounds[i]) { this->upper_init_ranges[i] = upper_param_bounds[i]; }
        }
    ...
    /**
     * Clones the current ADAM object.
     * For the 'adam_on_population' object please remember that the solution are kept there,
     * where only the generate next solution method is used from this object.
     * Comment run warning means that if method Run() is invoked, the variable will be unintentionally changed
     * @return A clone of this object
     */
    adam_pt adam_t::clone() {
        // Create a new optimizer
        /*
         * Variables taken care of: version, use_momentum_with_nag, use_boundary_repair, accept_only_improvements,
         *  b1, b2, epsilon, finite_differences_multiplier, rng, lower_init_ranges, upper_init_ranges,
         *  lower_param_bounds, upper_param_bounds, maximum_number_of_evaluations, maximum_number_of_seconds,
         *  vtr, use_vtr, random_seed, write_generational_solutions, write_generational_statistics,
         *  write_directory, file_appendix
         */
        adam_pt cloneOptimizer = std::make_shared<adam_t>(
                fitness_function, version, lower_init_ranges, upper_init_ranges,
                maximum_number_of_evaluations, maximum_number_of_seconds, vtr, use_vtr, random_seed,
                write_generational_solutions, write_generational_statistics,
                write_directory, file_appendix, 0, finite_differences_multiplier);

        // Copy data
        cloneOptimizer->maximum_no_improvement_stretch = maximum_no_improvement_stretch;    // Run warning
        cloneOptimizer->no_improvement_stretch = no_improvement_stretch;
        cloneOptimizer->gamma = gamma;  // Run warning
        cloneOptimizer->best = best;
        cloneOptimizer->sol_current = sol_current;

        // Change optimization data (Probably not used in 'adam_on_population')
        cloneOptimizer->terminated = terminated;
        cloneOptimizer->success = success;
        cloneOptimizer->number_of_evaluations = number_of_evaluations;
        ...
        cloneOptimizer->starting_time = starting_time;
...
//    }
    ...
    void adam_t::run() {
        //---------------------------------------------
        // Reset the run's log variables (in case hillvallea is run multiple time)
        starting_time = clock();
        success = false;
        terminated = false;
        number_of_evaluations = 0;
        ...
        while (!checkTerminationCondition(old_fitness, *sol, no_improvement_stretch)) {
            ...
            if (write_generational_solutions) {
                std::stringstream ss;
                ss << write_directory << "best_generation" << std::setw(5) << std::setfill('0') << number_of_generations
                   << file_appendix << ".dat";
                fitness_function->write_solution(best, ss.str());
                std::stringstream ss_current;
                ss_current << write_directory << "current_generation" << std::setw(5) << std::setfill('0')
                           << number_of_generations << file_appendix << ".dat";
            ...
            if (version == 64) {
                ...
                weighted_number_of_evaluations += plainGradientOffspring(sol, touched_parameter_idx, gammas);
            } else if (version == 81) {
                weighted_number_of_evaluations += lineSearchOffspring(sol, touched_parameter_idx, gammas);
            } else {
    ...
    double adam_t::gradientOffspring(solution_pt &sol, const std::vector <std::vector<size_t>> &touched_parameter_idx,
                                     vec_t &gamma) {
        size_t number_of_parameters = fitness_function->number_of_parameters;
        ...
        for (size_t oj = 0; oj < touched_parameter_idx.size(); ++oj) {
            ...
            for (size_t i = 0; i < touched_parameter_idx[FOS_idx].size(); ++i) {
                ...
                if (use_adamax) {
                    ...
                    // Calculation
                    sol->adam_vt[param_i] =
                            b2 * sol->adam_vt[param_i] + (1.0 - b2) * (sol->gradient[param_i] * sol->gradient[param_i]);

                    // Calculate v^t / (1-b_2 ^ t + 1)
                    vt_biased[i] = sol->adam_vt[param_i] / (1.0 - pow(b2, number_of_generations + 1.0));

                    // Calculate sqrt(v^t / (1-b_2 ^ t + 1) + epsilon)
                    gamma_factor = sqrt(vt_biased[i]) + epsilon;
                }

                // Calculate step
                if (use_momentum_with_nag) {
                    step[i] = -(gamma[FOS_idx] * b1) * (mt_biased[i] / gamma_factor) -
                              (gamma[FOS_idx] * (1.0 - b1) / (1.0 - pow(b1, number_of_generations + 1.0))) *
                              (sol->gradient[param_i] / gamma_factor);
                } else {
                    step[i] = -gamma[FOS_idx] * (mt_biased[i] / gamma_factor);
                }
            }

            // Apply step while keeping the displacement within the range
            bool out_of_range = true;
            double shrink_factor = 2;
            while (out_of_range && (shrink_factor > epsilon)) {
                // Keep shrinking the step size until all parameter are within bounds
                shrink_factor *= 0.5;
                out_of_range = false;
            ...
            if (!out_of_range) {
                ...
                // Copy old solution
                old_sol = std::make_shared<solution_t>(
                        *sol); // this still copies the entire solution, which is probably not desirable


                // Apply changes to the current solution
                for (size_t i = 0; i < touched_parameter_idx[FOS_idx].size(); i++) {
                    size_t param_i = touched_parameter_idx[FOS_idx][i];
            ...
            while (!found_improvement && (shrink_factor > epsilon)) {
                ...
                if (touched_parameter_idx[FOS_idx].size() == number_of_parameters) {
                    ...
                    if (!fitness_function->use_finite_differences) {
                        ...
                        fitness_function->evaluate_with_finite_differences(sol, finite_differences_multiplier *
                                                                                gamma[FOS_idx]);
                        weighted_number_of_evaluations++;
                    }
                } else {
                    ...
                    if (!fitness_function->use_finite_differences) {
                        ...
                        weighted_number_of_evaluations += (touched_parameter_idx[FOS_idx].size() /
                                                           (double) number_of_parameters);
                    } else {
                        fitness_function->partial_evaluate_with_finite_differences(sol, touched_parameter_idx[FOS_idx],
                                                                                   old_sol,
                ...
                // Update parameters when new solution is worse
                if (solution_t::better_solution(*old_sol, *sol)) {
                    ...
                    if (accept_only_improvements) {
                        ...
                        gamma[FOS_idx] *= stepSizeShrinkFactor; // 1 - 0.1 / (double) sol->mo_test_sols.size();
                    }
                } else {
                    no_improvement_stretch = 0;
                    found_improvement = true;   // Used to exit loop
                }
            }

            // Reset old solution when worse solution found
    ...
    double
    adam_t::HIGAMOgradientOffspring(solution_pt &sol, const std::vector <std::vector<size_t>> &touched_parameter_idx,
                                    vec_t &gamma) {
        ...
        solution_pt old_sol = std::make_shared<solution_t>(
                *sol); // this still copies the entire solution, which is probably not desirable
    ...
    double
    adam_t::plainGradientOffspring(solution_pt &sol, const std::vector <std::vector<size_t>> &touched_parameter_idx,
                                   vec_t &gamma) {
        ...
        for (size_t oj = 0; oj < touched_parameter_idx.size(); ++oj) {
            ...
            // Prepare normalization of gradient
            double sum_gradient_square = 0;
            for (const auto &gradient_i : sol->gradient) {
                sum_gradient_square += (gradient_i *  gradient_i);
            }
            double abs_gradient = sqrt(sum_gradient_square);
            ...
            for (size_t i = 0; i < touched_parameter_idx[FOS_idx].size(); ++i) {
                ...
                // Calculate step
                step[i] = -gamma[FOS_idx] * (sol->gradient[param_i]); // abs_gradient
            }
    ...
    double
    adam_t::lineSearchOffspring(solution_pt &sol, const std::vector <std::vector<size_t>> &touched_parameter_idx,
                                   vec_t &gamma) {
        // Implement things here
        size_t number_of_parameters = fitness_function->number_of_parameters;
        double weighted_number_of_evaluations = 0;
        ...
        for (size_t oj = 0; oj < touched_parameter_idx.size(); ++oj) {
            ...
            // Prepare variables
            vec_t step(touched_parameter_idx[FOS_idx].size(), 0.0);
            vec_t stepDirection(touched_parameter_idx[FOS_idx].size(), 0.0);
            vec_t new_param(touched_parameter_idx[FOS_idx].size(), 0.0);
            ...
            for (size_t i = 0; i < touched_parameter_idx[FOS_idx].size(); ++i) {
                ...
                // Include step factor in step calculation
                stepDirection[i] = -gamma[FOS_idx] * (sol->gradient[param_i]);
            }

            // Keep track of solutions
            solution_pt currentSolution = std::make_shared<solution_t>(*sol);
            solution_pt bestCandidateSolution;
            double bestStepFactor;

            // Try out different step sizes
            double step_factors[] = { 0.01, 0.05, 0.1, 0.5, 0.99, 1, 1.5, 2};
            bestStepFactor = step_factors[0];
            for (const auto &step_factor : step_factors){
                // Apply step factor
                for (size_t i = 0; i < touched_parameter_idx[FOS_idx].size(); ++i) {
                    step[i] = step_factor * stepDirection[i];
                }
                ...
                while (out_of_range && (shrink_factor > epsilon)) {
                    ...
                    // Apply step with shrink factor
                    for (size_t i = 0; i < touched_parameter_idx[FOS_idx].size(); i++) {
                        // Apply the factor
                        size_t param_i = touched_parameter_idx[FOS_idx][i];
                        new_param[i] = currentSolution->param[param_i] + shrink_factor * step[i];
                ...
                if (!out_of_range) {
                    ...
                    // Create new solution
                    candidateSolution = std::make_shared<solution_t>(*currentSolution);

                    // Apply changes to the candidate solution
                    for (size_t i = 0; i < touched_parameter_idx[FOS_idx].size(); i++) {
                        size_t param_i = touched_parameter_idx[FOS_idx][i];
                        candidateSolution->param[param_i] = new_param[i];
                    }
                } else {
                    // terminate the loop;
                    std::cout << "Solution out of range, skip update." << std::endl;
                ...
                if (touched_parameter_idx[FOS_idx].size() == number_of_parameters) {
                    // Full evaluation
                    if (!fitness_function->use_finite_differences) {
                        fitness_function->evaluate_with_gradients(candidateSolution);
                        weighted_number_of_evaluations++;
                    } else {
                        fitness_function->evaluate_with_finite_differences(
                                candidateSolution,
                                finite_differences_multiplier * gamma[FOS_idx]);
                        weighted_number_of_evaluations++;
                    }
                } else {
                    // Partial evaluation
                    if (!fitness_function->use_finite_differences) {
                        fitness_function->partial_evaluate_with_gradients(candidateSolution,
                                                                          touched_parameter_idx[FOS_idx],
                                                                          currentSolution);
                        weighted_number_of_evaluations += (touched_parameter_idx[FOS_idx].size() /
                                (double) number_of_parameters);
                    } else {
                        fitness_function->partial_evaluate_with_finite_differences(
                                candidateSolution,
                                touched_parameter_idx[FOS_idx],
                                currentSolution,
                                finite_differences_multiplier * gamma[FOS_idx]);
                ...
                // Assign the best candidate solution and step factor used to create this solution
                if (bestCandidateSolution == nullptr ||
                        solution_t::better_solution(*candidateSolution, *bestCandidateSolution)) {
                    bestCandidateSolution = candidateSolution;
                    bestStepFactor = step_factor;
                }
            }

            // Assign best solution
            if (solution_t::better_solution(*bestCandidateSolution, best)) {
                best = *bestCandidateSolution;
            }

            // Update parameters for next iteration
            if (solution_t::better_solution(*currentSolution, *bestCandidateSolution)) {
                no_improvement_stretch++;
                if (accept_only_improvements) {
                    throw std::runtime_error("Forced improvements has not been implemented for LINE SEARCH APP.\n");
//                        shrink_factor *= 0.5;
//                        found_improvement = false;
//
//                        for (size_t i = 0; i < touched_parameter_idx[FOS_idx].size(); i++) {
...
//                        }
                    ...
                    // Shrink step size to the smallest factor
                    gamma[FOS_idx] *= step_factors[0];
                }
            } else {
                no_improvement_stretch = 0;

                // Update the step size
                gamma[FOS_idx] *= bestStepFactor;
            }

            // Reset old solution
            sol = bestCandidateSolution;
    ...
    double adam_t::evaluateWithfiniteDifferences(solution_pt &sol, double h, bool use_central_difference) const {
        ...
        for (size_t i = 0; i < sol->param.size(); ++i) {
            ...
            if (use_central_difference) {
                ...
                std::cout << " --> " << sol->gradient[i] << ". Error = "
                          << (sol->gradient[i] - old_gradient) / old_gradient << std::endl;
            }
    ...
    double adam_t::partialEvaluateWithfiniteDifferences(solution_pt &sol, double h, bool use_central_difference,
                                                        const std::vector <size_t> &touched_parameter_idx,
                                                        const solution_pt &old_sol) const {
        double weighted_number_of_evaluations = 0;
