...
namespace hillvallea {
    ...
    void hillvallea_t::init_default_params() {
        // Set parameters of the recursion scheme
        ...
        // Set single objective parameter bounds
    ...
    void hillvallea_t::new_statistics_file() {
        ...
        statistics_file << "Pop  Cluster    Gen   Evals        Time No.Elites  Best-elite   Average-obj       Std-obj"
                        << std::endl;

    }

    /**
     * Create serial statistics file. Todo: Find out why it's implemented like this???
     */
    ...
    /**
     * Create serial statistics file. Todo: Find out why it's implemented like this????
     * @param use_clustering Should we use clustering
     */
...
//        assert(statistics_file.is_open());
    ...
    /**
     * Todo: Find out why this is different from serial?
     */
    void hillvallea_t::write_statistics_line_population(const population_t &pop,
                                                        const std::vector<optimizer_pt> &local_optimizers,
                                                        const std::vector<solution_pt> &elitist_archive) {
    ...
    /**
     * Todo: Again why like this????
     */
    void hillvallea_t::write_statistics_line_serial(const population_t &pop, size_t number_of_generations,
                                                    const solution_t &best) {
        write_statistics_line_serial(pop, number_of_generations, best, false);
    }

    /**
     * Write results of the population to the statistics file.
     * @param pop  The population to write about.
     * @param number_of_generations The current generation number
     * @param best The best solution
     * @param use_clustering Boolean if clustering is used
     */
    void hillvallea_t::write_statistics_line_serial(const population_t &pop, size_t number_of_generations,
                                                    const solution_t &best, bool use_clustering) {
        ...
        statistics_file
                ...
                << " " << std::setw(13) << std::scientific << std::setprecision(16) << pop.average_fitness()
                ...
                << " " << fitness_function->write_additional_solution_info(best, elitist_archive, use_clustering)
                << std::endl;
    }

    /**
     * Todo: I don't think this is useful for me
     */
    void hillvallea_t::write_statistics_line_cluster(const population_t &cluster_pop, int cluster_number,
                                                     int cluster_generation,
                                                     const std::vector<solution_pt> &elitist_archive) {
    ...
    /**
     * Close the statistics file. Todo: Why like this?
     */
    ...
    //-------------------------------------------------------------------------------
    /**
     * Write the current population to a file. Todo: This function seems not to be used.
     * @param pop The population to write to a file
     * @param local_optimizers The local optimizers used.
     */
    void hillvallea_t::write_population_file(population_pt pop, std::vector<optimizer_pt> &local_optimizers) const {
        std::ofstream file;
        std::stringstream ss;
        ss << write_directory << "population" << std::setw(5) << std::setfill('0') << number_of_generations
           << "_inital_population" << file_appendix << ".dat";
    ...
    //-------------------------------------------------------------------------------
    /**
     * Writes the population of a cluster
     * @param generation_number The current generation number
     * @param cluster_number The number of the cluster
     * @param cluster_generation The generation of the cluster
     * @param pop The population to write to a file
     */
    void hillvallea_t::write_cluster_population(int generation_number, size_t cluster_number, int cluster_generation,
                                                population_pt pop) const {
        std::ofstream file;
        std::stringstream ss;

        // Determine population file name
        ss << write_directory << "population" << std::setw(5) << std::setfill('0') << generation_number << "_cluster"
           << std::setw(5) << std::setfill('0') << cluster_number << "_generation" << cluster_generation
           << file_appendix << ".dat";
    ...
    /**
     * Write the elitist archive to a file
     * @param elitist_archive The elitist archive
     * @param final The last archive to be written
     */
    void hillvallea_t::write_elitist_archive_file(const std::vector<solution_pt> &elitist_archive, bool final) const {
        ...
        // Write the content of the elitist archive
        int precision = std::numeric_limits<long double>::digits10 + 1;
        for (size_t i = 0; i < elitist_archive.size(); ++i) {
            file
                    << std::fixed << std::setw(precision + 5) << std::setprecision(precision + 1)
                    << elitist_archive[i]->param
                    << std::fixed << std::setw(precision + 5) << std::setprecision(precision + 1)
                    << elitist_archive[i]->f
                    << std::fixed << std::setw(precision + 5) << std::setprecision(precision + 1)
                    << elitist_archive[i]->constraint;

            if (i == 0 && use_vtr)
                file << " " << std::setprecision(std::numeric_limits<long double>::digits10 + 1) << vtr << " "
                     << success;
    ...
    void hillvallea_t::write_CEC2013_niching_file(bool final) {
        ...
        if (final) {
            std::stringstream ss;
            ss << write_directory << "niching_archive_core" << local_optimizer_index << "_cluster" << cluster_alg << "_"
               << fitness_function->name() << "_run" << std::setw(3) << std::setfill('0') << random_seed
               << "_final.dat";
            filename = ss.str();
            ...
            std::stringstream ss;
            ss << write_directory << "niching_archive_core" << local_optimizer_index << "_cluster" << cluster_alg << "_"
               << fitness_function->name() << "_run" << std::setw(3) << std::setfill('0') << random_seed << "_"
               << number_of_generations << ".dat";
            filename = ss.str();
        }
        ...
        for (size_t i = 0; i < elitist_archive.size(); ++i) {
            file
                    ...
                    << std::fixed << std::setw(precision + 5) << std::setprecision(precision + 1)
                    << elitist_archive[i]->time_obtained;
     ...
     * @param elite_candidates
     * @return
     */
    bool hillvallea_t::terminate_on_approaching_elite(optimizer_t &local_optimizer,
    ...
    /**
     * Terminate when the population is converging to a local optimum
     * @param local_optimizer The local optimizers used
     * @param elite_candidates The elitist candidates
     * @return If the process should be terminated because it's optimizing towards a local optimum
     */
    bool hillvallea_t::terminate_on_converging_to_local_optimum(optimizer_t &local_optimizer,
                                                                std::vector<solution_pt> &elite_candidates) {
        ...
        if (local_optimizer.pop->sols[0]->f > best + TargetTolFun) {
            ...
            for (size_t i = 0; i < lookback_window - 1; ++i) {
                if (local_optimizer.average_fitness_history[local_optimizer.average_fitness_history.size() -
                                                            lookback_window + i]
                    < local_optimizer.average_fitness_history[local_optimizer.average_fitness_history.size() -
                                                              lookback_window + i + 1]) {
                    return false;
                }
            }
            ...
            double prev_fitness = local_optimizer.average_fitness_history[
                    local_optimizer.average_fitness_history.size() - lookback_window];
    ...
    void hillvallea_t::initialize(population_pt pop, size_t population_size, double selection_fraction_multiplier,
                                  std::vector<optimizer_pt> &local_optimizers,
                                  const std::vector<solution_pt> &elitist_archive, bool enable_clustering,
                                  size_t target_clustersize) {
        ...
        if (fitness_function->redefine_random_initialization) {
            fitness_function->init_solutions_randomly(*pop, population_size, lower_init_ranges, upper_init_ranges, 0,
                                                      rng);
        } else {
        ...
        optimizer_pt local_optimizer = init_optimizer(local_optimizer_index, number_of_parameters, lower_param_bounds,
                                                      upper_param_bounds,
                                                      init_univariate_bandwidth, fitness_function,
                                                      rng);
        ...
        for (auto cluster = clusters.begin(); cluster != clusters.end(); ++cluster) {
            if ((*cluster)->sols.size() > 0) {
                optimizer_pt opt = init_optimizer(local_optimizer_index, number_of_parameters, lower_param_bounds,
                                                  upper_param_bounds,
    ...
    void hillvallea_t::run() {
        ...
        {
            hillvallea::optimizer_pt dummy_optimizer = init_optimizer(local_optimizer_index, number_of_parameters,
                                                                      lower_param_bounds, upper_param_bounds,1.0,
                                                                      fitness_function, rng);
            current_cluster_size =
                    cluster_size_initializer * dummy_optimizer->recommended_popsize(number_of_parameters);
        }
        ...
        while (restart) {
            ...
            if (maximum_number_of_evaluations > 0 &&
                current_population_size > (maximum_number_of_evaluations - number_of_evaluations)) {
                restart = false;
                break;
            }
            ...
            initialize(pop, (size_t) current_population_size, current_selection_fraction_multiplier, local_optimizers,
                       elitist_archive, true, current_cluster_size);
            ...
            for (size_t i = 0; i < local_optimizers.size(); ++i) {
                ...
                while (true) {
                    ...
                    size_t fevals_needed_to_check_elites = 1 +
                                                           (size_t) (elite_candidates.size() * add_elites_max_trials *
                                                                     (elitist_archive.size() +
                                                                      elite_candidates.size() * 0.5));

                    if (maximum_number_of_evaluations > 0 &&
                        number_of_evaluations + fevals_needed_to_check_elites + current_cluster_size >=
                    ...
                    if (use_vtr && local_optimizers[i]->pop->size() != 0) {
                        ...
                        if (!fitness_function->redefine_vtr) {
                            vtr_hit = (local_optimizers[i]->pop->sols[0]->constraint == 0) &&
                                      (local_optimizers[i]->pop->sols[0]->f <= vtr);
                        } else {
                    ...
                    if (local_optimizers[i]->active && local_optimizers[i]->checkTerminationCondition()) {
                        if (local_optimizers[i]->pop->size() > 0) {
                            if (elitist_archive.size() == 0 ||
                                local_optimizers[i]->pop->sols[0]->f < elitist_archive[0]->f + TargetTolFun) {
                                elite_candidates.push_back(local_optimizers[i]->pop->sols[0]);
                    ...
                    if (local_optimizers[i]->active) {
                        ...
                        int local_number_of_evaluations = (int) local_optimizers[i]->sample_new_population(
                                (size_t) current_cluster_size);
                        number_of_evaluations += local_number_of_evaluations;

                        if (write_generational_solutions) {
                            write_cluster_population(number_of_generations, i,
                                                     local_optimizers[i]->number_of_generations,
                                                     local_optimizers[i]->pop);
                        }

                        local_optimizers[i]->pop->truncation_percentage(*local_optimizers[i]->pop,
                                                                        local_optimizers[i]->selection_fraction);
                        local_optimizers[i]->average_fitness_history.push_back(
                                local_optimizers[i]->pop->average_fitness());

                        if (write_generational_statistics) {
                            write_statistics_line_cluster(*local_optimizers[i]->pop, (int) i,
                                                          local_optimizers[i]->number_of_generations, elitist_archive);
                        }
                    }
                }
            }
            ...
            add_elites_to_archive(elitist_archive, elite_candidates, number_of_global_opts_found,
    ...
    void hillvallea_t::runSerial() {
        ...
        // Init population sizes
        //---------------------------------------------
        hillvallea::optimizer_pt optimizer = init_optimizer(local_optimizer_index, number_of_parameters,
                                                            lower_param_bounds, upper_param_bounds, 1.0,
                                                            fitness_function, rng);
        ...
        if (fitness_function->redefine_random_initialization) {
            fitness_function->init_solutions_randomly(*optimizer->pop, population_size, lower_init_ranges,
                                                      upper_init_ranges, 0, rng);
        } else {
            optimizer->pop->fill_uniform(population_size, number_of_parameters, lower_init_ranges, upper_init_ranges,
                                         rng);
        }
        ...
        population_pt selection = std::make_shared<population_t>();
        optimizer->pop->truncation_percentage(*selection,
                                              optimizer->selection_fraction * selection_fraction_multiplier);
        ...
        */
        ...
        while (optimizer->active) {
            ...
            if (optimizer->active) {
                ...
                if (write_generational_solutions) {
                    write_cluster_population(number_of_generations, 0, optimizer->number_of_generations,
                                             optimizer->pop);
                }
    ...
    void hillvallea_t::runSerial2(size_t popsize, bool enable_clustering, double local_optima_tolerance) {
        ...
        while (restart) {
            ...
            // compute initial population
            initialize(pop, (size_t) current_population_size, current_selection_fraction_multiplier, local_optimizers,
                       elitist_archive, enable_clustering, current_cluster_size);

            if (write_generational_statistics) {
                write_statistics_line_serial(*pop, number_of_generations, *pop->sols[0], enable_clustering);
            }

            std::cout << "Restart with a popsize of " << current_population_size << ", located "
                      << local_optimizers.size() << " niche(s)." << std::endl;
            ...
            for (size_t i = 0; i < local_optimizers.size(); ++i) {
                ...
                while (true) {
                    if (maximum_number_of_evaluations > 0 && number_of_evaluations >= maximum_number_of_evaluations) {
                        std::cout << "  Terminated core search algorithm because function evaluations limit reached"
                                  << std::endl;
                        ...
                        if (vtr_hit) {
                            ...
                            if (fitness_function->partial_evaluations_available &&
                                ...
                                for (hit_idx = 0; hit_idx < local_optimizers[i]->pop->size(); ++hit_idx) {
                                    ...
                                    // recheck
                                    if (!fitness_function->redefine_vtr) {
                                        if ((local_optimizers[i]->pop->sols[hit_idx]->constraint != 0) ||
                                            (local_optimizers[i]->pop->sols[hit_idx]->f > vtr)) {
                                            break;
                                        }
                                    } else {
                                    ...
                                    if (!fitness_function->redefine_vtr) {
                                        true_hit = (local_optimizers[i]->pop->sols[0]->constraint == 0) &&
                                                   (local_optimizers[i]->pop->sols[0]->f <= vtr);
                                    } else {
                                        true_hit = fitness_function->vtr_reached(*local_optimizers[i]->pop->sols[0],
                                                                                 vtr);
                                    }
                    ...
                    if ((1 + local_optimizers[i]->number_of_generations) % 5 == 0) {
                        if (terminate_on_approaching_elite(*local_optimizers[i], elite_candidates)) {
                            std::cout << "  Terminated core search algorithm because approaching elite (f = "
                                      << local_optimizers[i]->pop->sols[0]->f << ", x[0] = "
                                      << local_optimizers[i]->pop->sols[0]->param[0] << ")." << std::endl;
                            local_optimizers[i]->active = false;
                            break;
                        }
                    }
                    ...
                    // stop this local optimizer if it converges to a local optimum
                    if (terminate_on_converging_to_local_optimum(*local_optimizers[i], elite_candidates)) {
                        std::cout << "  Terminated core search algorithm because approaching local optimum"
                                  << std::endl;
                        local_optimizers[i]->active = false;
                        break;
                    }
                    ...
                    // we add the best solution to the elitist archive
                    if (local_optimizers[i]->active && local_optimizers[i]->checkTerminationCondition()) {
                        std::cout << "  Terminated core search algorithm because of internal reasons (f = "
                                  << local_optimizers[i]->pop->sols[0]->f << ", x[0] = "
                                  << local_optimizers[i]->pop->sols[0]->param[0] << ")." << std::endl;
                        if (local_optimizers[i]->pop->size() > 0) {
                            if (elitist_archive.size() == 0 ||
                    ...
                    if (local_optimizers[i]->active) {
                        ...
                        if (write_generational_statistics && (local_optimizers[i]->number_of_generations < 50 ||
                                                              local_optimizers[i]->number_of_generations % 50 == 0)) {
                            if (local_optimizers[i]->pop->size() ==
                                current_cluster_size) { // GOMEA keeps the population in pop, while AMaLGaM only maintains the selection. This is therefore a bit of a lousy hack.
                                population_t selection;
                                local_optimizers[i]->pop->truncation_percentage(selection,
                                                                                local_optimizers[i]->selection_fraction);
                                // write_statistics_line_cluster(selection, (int) i, local_optimizers[i]->number_of_generations, elitist_archive);
                                write_statistics_line_serial(selection, local_optimizers[i]->number_of_generations,
                                                             *local_optimizers[i]->pop->sols[0], enable_clustering);
                            } else {
                                // write_statistics_line_cluster(*local_optimizers[i]->pop, (int) i, local_optimizers[i]->number_of_generations, elitist_archive);
                                write_statistics_line_serial(*local_optimizers[i]->pop,
                                                             local_optimizers[i]->number_of_generations,
                                                             *local_optimizers[i]->pop->sols[0], enable_clustering);
                            }
                        }
                        ...
                        if (((int) (number_of_evaluations / 10000)) != best_written_prev_fevals) {
                            ...
                            std::stringstream ss;
                            ss << write_directory << "best" << std::setw(5) << std::setfill('0')
                               << best_written_prev_fevals << file_appendix << ".dat";
            ...
            if (write_generational_statistics && !enable_clustering) {
                ...
                {
                    if (local_optimizers[0]->pop->size() ==
                        current_cluster_size) { // GOMEA keeps the population in pop, while AMaLGaM only maintains the selection. This is therefore a bit of a lousy hack.
                        population_t selection;
                        local_optimizers[0]->pop->truncation_percentage(selection,
                                                                        local_optimizers[0]->selection_fraction);
                        // write_statistics_line_cluster(selection, (int) i, local_optimizers[i]->number_of_generations, elitist_archive);
                        write_statistics_line_serial(selection, local_optimizers[0]->number_of_generations,
                                                     *local_optimizers[0]->pop->sols[0], enable_clustering);
                    } else {
                        // write_statistics_line_cluster(*local_optimizers[i]->pop, (int) i, local_optimizers[i]->number_of_generations, elitist_archive);
                        write_statistics_line_serial(*local_optimizers[0]->pop,
                                                     local_optimizers[0]->number_of_generations,
                                                     *local_optimizers[0]->pop->sols[0], enable_clustering);
                    }
                }
            }
            ...
            add_elites_to_archive(elitist_archive, elite_candidates, number_of_global_opts_found,
                                  number_of_new_global_opts_found);
            std::cout << "  Of " << elite_candidates.size() << " elite candidates, found "
                      << number_of_new_global_opts_found
                      << ((number_of_new_global_opts_found != 1) ? " new optima." : " new optimum.")
                      << " The elitist archive now contains " << elitist_archive.size()
                      << (elitist_archive.size() == 1 ? " solution." : " solutions.") << std::endl;
     ...
     * Todo: No idea why it's called runSerial3
     * @param popsize
     * @param enable_clustering
     * @param initial_number_of_evaluations
     */
    void hillvallea_t::runSerial3(size_t popsize, bool enable_clustering, int initial_number_of_evaluations) {
        ...
        else if (local_optimizer_index == 500 || local_optimizer_index == 640 ||
            local_optimizer_index == 64500 || local_optimizer_index == 64501 ||
            local_optimizer_index == 66500 || local_optimizer_index == 66501 ||
            local_optimizer_index == 50500 || local_optimizer_index == 50501 ||
            local_optimizer_index == 64640 || local_optimizer_index == 64641 ||
            local_optimizer_index == 66640 || local_optimizer_index == 66641 ||
            local_optimizer_index == 50640 || local_optimizer_index == 50641)
        {
            evalute_with_gradients = true;
        }
        ...
        double current_cluster_size = popsize;
        double current_population_size = enable_clustering ? pow(2.0, population_size_initializer)
                                                           : current_cluster_size;
        double current_selection_fraction_multiplier = 1.0;

        // Determine recommended cluster size
        if (popsize == 0) {
            hillvallea::optimizer_pt dummy_optimizer = init_optimizer(local_optimizer_index, number_of_parameters,
                                                                      lower_param_bounds, upper_param_bounds, 1.0,
                                                                      fitness_function, rng);
        ...
        //--------------------------------------------
        while (restart) {
            // Disable restart if clustering not enabled
            if (!enable_clustering) {
                restart = false;
            }
            ...
            // Terminate if the initial population size is too large.
            // Todo: There is a bit to gain here by decreasing the popsize so that we can run some more local opts, but I don't think its worth it.
            if (maximum_number_of_evaluations > 0 &&
                current_population_size > (maximum_number_of_evaluations - number_of_evaluations)) {
                restart = false;
                std::cout << "Terminated because max function evaluations is (almost) reached." << std::endl;
                break;
            }

            // Initialize the population
            if (fitness_function->redefine_random_initialization) {
                fitness_function->init_solutions_randomly(*pop, (size_t) current_population_size, lower_init_ranges,
                                                          upper_init_ranges, 0, rng);
            } else {
                pop->fill_uniform((size_t) current_population_size, number_of_parameters, lower_init_ranges,
                                  upper_init_ranges, rng);
            }
            ...
            optimizer_pt opt = init_optimizer(local_optimizer_index, number_of_parameters, lower_param_bounds,
                                 upper_param_bounds, init_univariate_bandwidth, fitness_function, rng);

            opt->initialize_from_population(pop, pop->size());
            opt->average_fitness_history.push_back(opt->pop->average_fitness());
            local_optimizers.push_back(opt);

            // Set tolerance when clustering
            if (enable_clustering) {
                opt->param_std_tolerance = 1e-8;
                opt->fitness_std_tolerance = 1e-8;
                opt->penalty_std_tolerance = 1e-8;
            ...
            for (size_t i = 0; i < local_optimizers.size(); ++i) {
                ...
                while (true) {
                    ...
                    if (terminate_on_runtime()) {
                        ...
                        // Add local optimizers best solution to candidate elite solutions.
                        if (local_optimizers[i]->pop->size() > 0) {
                            elite_candidates.push_back(local_optimizers[i]->pop->sols[0]);
                        }
                        break;
                    ...
                    // Terminate if VTR is hit
                    if (use_vtr && local_optimizers[i]->pop->size() > 0) {

                        // Determine if VTR has been reached
                        bool vtr_hit = false;
                        if (!fitness_function->redefine_vtr) {
                            vtr_hit = (local_optimizers[i]->pop->sols[0]->constraint == 0) &&
                                      (local_optimizers[i]->pop->sols[0]->f <= vtr);
                    ...
                    // Terminate local optimizer if it approaches a previously obtained elite (candidate)
                    if ((1 + local_optimizers[i]->number_of_generations) % 5 == 0) {
                        if (terminate_on_approaching_elite(*local_optimizers[i], elite_candidates)) {
                    ...
                    // Terminate local optimizer if it converges to a local optimum
                    if (terminate_on_converging_to_local_optimum(*local_optimizers[i], elite_candidates)) {
                        std::cout << "  Terminated core search algorithm because approaching local optimum"
                                  << std::endl;
                        local_optimizers[i]->active = false;
                    ...
                    if (local_optimizers[i]->active) {
                        ...
                        // Write the generational solutions
                        if (write_generational_solutions) {
                            write_cluster_population(number_of_generations, i,
                                                     local_optimizers[i]->number_of_generations,
                                                     local_optimizers[i]->pop);
                        ...
                        // Write the generational statistics
//                        if (write_generational_statistics && (local_optimizers[i]->number_of_generations < 100 ||
//                                                              local_optimizers[i]->number_of_generations % 100 == 0))
                        {
                ...
                } // End of while-Local optimizer

                // Stop applying local optimizers if it's not clustering
                if (!enable_clustering) {
                    break;
                }

                // Stop applying local optimizers if we're not supposed to reset/ when we've reached success
                if (!restart || success) {
                    break;
                }
            }

            // Todo: Write final generational statistics?
            if (write_generational_statistics && !enable_clustering) {
                if (local_optimizers[0]->number_of_generations > 50 &&
                    local_optimizers[0]->number_of_generations % 100 != 0) {
                    if (local_optimizers[0]->pop->size() ==
                        current_cluster_size) { // GOMEA keeps the population in pop, while AMaLGaM only maintains the selection. This is therefore a bit of a lousy hack.
                        population_t selection;
                        local_optimizers[0]->pop->truncation_percentage(selection,
            ...
            std::cout << "  Of " << elite_candidates.size() << " elite candidates, found "
                      << number_of_new_global_opts_found
                      << ((number_of_new_global_opts_found != 1) ? " new optima." : " new optimum.") << std::endl;
            ...
            // if we found no new global opt, this is either due to the fact that there are no new basins found,
            // or because the cluster size is too small.  increase both
            if (number_of_new_global_opts_found == 0) {
                current_cluster_size *= cluster_size_incrementer;
                current_population_size *= population_size_incrementer;
                current_selection_fraction_multiplier *= current_selection_fraction_multiplier;
        ...
        // Sort the elitist archive where the best solutions are ranked first
        if (elitist_archive.size() > 0) {
            best = solution_t(*elitist_archive[0]);
        }
    ...
    void hillvallea_t::add_elites_to_archive(std::vector<solution_pt> &elitist_archive,
                                             const std::vector<solution_pt> &elite_candidates,
                                             int &number_of_global_opts_found, int &number_of_new_global_opts_found) {
            ...
            if (elitist_archive.size() > 0) {
                ...
                if (check_edge(*elitist_archive[j], *potential_candidates[i], add_elites_max_trials)) {
                    ...
                    if (solution_t::better_solution_via_pointers(potential_candidates[i], elitist_archive[j])) {
                        ...
                        elitist_archive[j]->time_obtained =
                                ((double) (clock() - starting_time)) / CLOCKS_PER_SEC * 1000.0;
...
bool hillvallea::hillvallea_t::check_edge(const hillvallea::solution_t &sol1, const hillvallea::solution_t &sol2,
                                          int max_trials) {
...
bool hillvallea::hillvallea_t::check_edge(const hillvallea::solution_t &sol1, const hillvallea::solution_t &sol2,
                                          int max_trials, std::vector<solution_pt> &test_points) {
...
void hillvallea::hillvallea_t::hillvalley_clustering(population_t &pop, std::vector<population_pt> &clusters,
                                                     size_t max_number_of_neighbours, bool add_test_solutions,
                                                     bool skip_check_for_worst_solutions,
                                                     bool check_all_neighbors_from_same_clusters) {
    ...
    for (size_t i = 1; i < pop.size(); i++) {
        ...
        for (size_t j = 0; j < std::min(i, max_number_of_neighbours); j++) {
            ...
            if (force_accept || check_edge(*pop.sols[i], *pop.sols[nearest_better_index], max_number_of_trial_solutions,
                                           new_test_points)) {
