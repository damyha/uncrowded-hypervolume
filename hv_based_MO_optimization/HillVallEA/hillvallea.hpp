...
namespace hillvallea {
    ...
    public:
        ...
        //--------------------------------------------------------------------------------
        void
        hillvalley_clustering(population_t &pop, std::vector<population_pt> &clusters, size_t max_number_of_neighbours,
                              bool add_test_solutions, bool skip_check_for_worst_solutions,
    ...
    private:
        ...
        // Run-time functions
        //-------------------------------------------------------------------------------
        void initialize(population_pt pop, size_t population_size, double selection_fraction_multiplier,
                        std::vector<optimizer_pt> &local_optimizers, const std::vector<solution_pt> &elitist_archive,
                        bool enable_clustering, size_t target_clustersize);

        void add_elites_to_archive(std::vector<solution_pt> &elitist_archive,
                                   const std::vector<solution_pt> &elite_candidates, int &global_opts_found,
                                   int &new_global_opts_found);
        ...
        bool terminate_on_converging_to_local_optimum(optimizer_t &local_optimizer,
        ...
        void
        write_statistics_line_serial(const population_t &pop, size_t number_of_generations, const solution_t &best);

        void
        write_statistics_line_population(const population_t &pop, const std::vector<optimizer_pt> &local_optimizers,
                                         const std::vector<solution_pt> &elitist_archive);

        void write_statistics_line_cluster(const population_t &cluster_pop, int cluster_number, int cluster_generation,
                                           const std::vector<solution_pt> &elitist_archive);
        ...
        void write_cluster_population(int generation_number, size_t cluster_number, int cluster_generation,
