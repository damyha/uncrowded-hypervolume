...
#include "../benchmark_functions/bi_exp_cos_decay.h"
...
void printUsage(void) {
    printf("Usage: uhv_gomea [-?] [-P] [-s] [-w] [-v] [-e] [-r] lmod pro dim ssize low upp pop ela eva sec vtr rnd wrp\n"); // [-n] [-f]
...
void parseParameters(int argc, char **argv, int *index) {
    ...
    noError = noError && sscanf(argv[*index + 11], "%d", &random_seed);
    write_directory = argv[*index + 12];
    noError = noError && sscanf(argv[*index + 13], "%lf", &f_alpha);
    noError = noError && sscanf(argv[*index + 14], "%lf", &f_beta);
    noError = noError && sscanf(argv[*index + 15], "%lf", &f_lambda);
    noError = noError && sscanf(argv[*index + 16], "%lf", &f_frequency);
...
/**
 * Assigns problem parameters to specific problems
 */
...
void checkOptions(void) {
    if (linkage_model_index == 0 || linkage_model_index == 1 || linkage_model_index == 2) {
        ...
        printf("\n");
        printf("Error: linkage model invalid (read: %d).", linkage_model_index);
        printf("\n\n");
    ...
    std::stringstream ss;
    ss << "_lmod" << linkage_model_index << "_problem" << problem_index << "_p" << number_of_reference_points << "_run"
...
void writeParametersFile() {
    ...
    std::string text = "";
    text += "Algorithm:UHV-GOMEA, ";
    text += "EAModel:" + std::to_string(linkage_model_index) + ", ";
    ...
    text += "popsize:" + std::to_string(popsize) + ", ";
...
int main(int argc, char **argv) {
    ...
    if (!use_bezier_interpolation) {
        fitness_function = std::make_shared<hillvallea::UHV_t>(mo_fitness_function, number_of_reference_points,
                                                               collect_all_mo_sols_in_archive,
                                                               elitist_archive_size_target, nullptr,
                                                               use_finite_differences);
    } else {
        fitness_function = std::make_shared<hillvallea::bezierUHV_t>(mo_fitness_function, number_of_reference_points,
    ...
    if (print_verbose_overview) {
        std::cout << "Optimizer settings: \n\tUHV-GOMEA\n\tlocal_optimizer_index = " << local_optimizer_index
                  ...
                  << number_of_reference_points << "\n\tso_number_of_parameters = "
                  << fitness_function->number_of_parameters << "\n\tuse_finite_differences = "
                  << (use_finite_differences ? "yes" : "no") << "\n\tpopsize = " << popsize << "\n\tenable_niching = "
                  << (enable_niching ? "yes" : "no") << "\n\trandom_seed = " << random_seed << "\n";
    }
    ...
    opt.runSerial3(popsize, enable_niching, 0);
    ...
    if (print_verbose_overview) {
        std::cout << "Best: \n\tHV = " << std::fixed << std::setprecision(14) << -opt.best.f << "\n\tMO-fevals = "
                  << opt.number_of_evaluations * number_of_reference_points << "\n\truntime = "
