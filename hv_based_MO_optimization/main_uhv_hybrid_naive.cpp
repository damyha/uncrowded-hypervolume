/**
 * UHV-HYBRID NAIVE
 * This algorithm applies the resource allocation scheme described in "Combining Gradient Techniques fir Numerical
 * Multi-Objective Evolutionary Optimization" without taking into account that the algorithms use data from previous
 * generations.
 *
 * The main difference between this and the traditional scripts is that this script uses a custom optimization function.
 * It strips away all the HillVallEA functionality and writes statistics for every (local) optimizer instead of
 * generational statistics
 *
 * Note: If you want to run the WFG benchmark using the correct initialization ranges [[0, 2], [0,4], [0, 8], ...]
 * You can achieve this by setting the range to [0, something way larger than 2*n] to hit the problem boundary.
 *
...
#include "HillVallEA/resource_allocation_scheme/resource_allocation_scheme_process.h"
#include "HillVallEA/resource_allocation_scheme/naive_resource_allocation_scheme.h"
...
bool tweakGradientAlgorithm;            // Boolean if extra parameters are passed to tweak the gradient algorithm
...
double initialStepEstimationFactor;     // Used to estimate the step size of the grad. alg.
double stepSizeDecayFactor;             // Used to shrink the step size if a better solution was not found
...
int improvementMetricIndex;             // Index of definition of improvement
 ...
 */
void printHelp() {
    printf("Usage: uhv_hybrid [-?] [-P] [-s] [-w] [-v] [-e] [-r] [-g] lmod vgrad mgrad imprv pro dim ssize low upp pop eva sec vtr ndec alpha beta lmbda freq rnd wrp [-g:est] [-g:shr]\n"); // [-n] [-f]
    ...
    printf(" -g: Extra arguments to tune the gradient algorithm.\n");
    ...
    printf("imprv: Index of function to use as improvement metric (0:Best f, 1: Avg f, 2: UHV improv (sol), 3: UHV improv (pop). \n");
    ...
    printf(" ndec: The Memory-decay value (value:[0 - 1]).\n");
    ...
    printf("[-g:est]: Percentage of search volume to use as initial guess of the step size. Default = 0.01.\n");
    printf("[-g:shr]: Percentage of step size to shrink if better solution not found. Default = 0.99.\n");
}
...
void printComputationSettings(const hillvallea::fitness_pt &singleObjectiveFitnessFunction) {
    ...
    std::cout << "\tImprovement Metric Index = " << improvementMetricIndex << std::endl;
    ...
    std::cout << "\tmemory decay factor = " << memoryDecayFactor << std::endl;
    std::cout << "\tenable_niching = " << (enableNiching ? "Yes" : "No") << std::endl;
    std::cout << "\ttweakGradientAlgorithm = " << (tweakGradientAlgorithm ? "Yes" : "No") << std::endl;
    std::cout << "\tinitialStepEstimationFactor = " << initialStepEstimationFactor << std::endl;
    std::cout << "\tstepSizeDecayFactor = " << stepSizeDecayFactor << std::endl;
    std::cout << "\trandom_seed = " << randomSeedNumber << std::endl;
}

void printComputationResults(const hillvallea::resource_allocation_scheme_process_pt process) {
    std::cout << "Best:" << std::endl;
    std::cout << "\tFitness = " << std::fixed << std::setprecision(14) << -process->uhvHybrid->best.f << std::endl;
    ...
    for (auto &mo_test_sol : process->uhvHybrid->best.mo_test_sols) {
...
void writeParametersFile() {
    ...
    text += "ImprovementMetricIndex:" + std::to_string(improvementMetricIndex) + ", ";
    ...
    text += "mdecay:" + std::to_string(memoryDecayFactor) + ", ";
    text += "initialStepEstimationFactor:" + std::to_string(initialStepEstimationFactor) + ", ";
    text += "stepSizeDecayFactor:" + std::to_string(stepSizeDecayFactor) + ", ";
...
void initializeOptions() {
    ...
    enableNiching = false;
    tweakGradientAlgorithm = false;
    useFiniteDifferenceApproximation = false;
    memoryDecayFactor = 0.75;
    initialStepEstimationFactor = 0.01;
    stepSizeDecayFactor = 0.99;
}
...
void parseParameters(int argc, char **argv, const int *index) {
    ...
    int totalNumberOfParameters = 20;

    // Check options
    if (tweakGradientAlgorithm) {
    ...
    noError = noError && sscanf(argv[*index + 3], "%d", &improvementMetricIndex);
    noError = noError && sscanf(argv[*index + 4], "%d", &problemIndex);
    noError = noError && sscanf(argv[*index + 5], "%zd", &numberOfMOParameters);
    noError = noError && sscanf(argv[*index + 6], "%zd", &solutionSetSize);
    noError = noError && sscanf(argv[*index + 7], "%lf", &lowerInitializationBound);
    noError = noError && sscanf(argv[*index + 8], "%lf", &upperInitializationBound);
    noError = noError && sscanf(argv[*index + 9], "%zd", &populationSize);
    noError = noError && sscanf(argv[*index + 10], "%d", &maximumNumberOfMOEvaluations);
    noError = noError && sscanf(argv[*index + 11], "%d", &maximumTimeInSeconds);
    noError = noError && sscanf(argv[*index + 12], "%lf", &valueVTR);
    noError = noError && sscanf(argv[*index + 13], "%lf", &memoryDecayFactor);
    noError = noError && sscanf(argv[*index + 14], "%lf", &factorAlpha);        // Problem setting
    noError = noError && sscanf(argv[*index + 15], "%lf", &factorBeta);         // Problem setting
    noError = noError && sscanf(argv[*index + 16], "%lf", &factorLambda);       // Problem setting
    noError = noError && sscanf(argv[*index + 17], "%lf", &factorFrequency);    // Problem setting
    noError = noError && sscanf(argv[*index + 18], "%d", &randomSeedNumber);
    writeDirectory = argv[*index + 19];

    // Look for more inputs
    if (tweakGradientAlgorithm) {
        noError = noError && sscanf(argv[*index + 20], "%lf", &initialStepEstimationFactor);
        noError = noError && sscanf(argv[*index + 21], "%lf", &stepSizeDecayFactor);
    }
...
void checkOptions() {
    ...
    if (indexApplicationMethodUHV_ADAM < 0 || indexApplicationMethodUHV_ADAM > 13 ) {
    ...
    // Check improvement metric index
    if (improvementMetricIndex < 0 || improvementMetricIndex > 5)
    {
        throw std::runtime_error("Unknown improvement metric method detected: " +
                                 std::to_string(improvementMetricIndex) + ".\n");
    }

    // Check that memory-decay value is valid
    if (memoryDecayFactor < 0 || memoryDecayFactor > 1) {
        throw std::runtime_error(std::string("Error: Decay factor not in valid range: " +
                                             std::to_string(memoryDecayFactor) + ").\n"));
    }

    // Check that the initial step size factor is in the correct range
    if (initialStepEstimationFactor <= 0) {
        throw std::runtime_error(std::string("Error: Initial step estimation factor not in valid range: " +
                                             std::to_string(initialStepEstimationFactor) + ").\n"));
    }

    // Check that the step size decay factor is in the correct range
    if (stepSizeDecayFactor <= 0) {
        throw std::runtime_error(std::string("Error: Step size decay factor not in valid range: " +
                                             std::to_string(stepSizeDecayFactor) + ").\n"));
    }
...
//    approximation_set_size = 0;
    ...
    std::stringstream ss;
    ss << "_UHV-HYBRID" << linkageModelUHV_GOMEA
...
int main(int argc, char **argv) {
    ...
    if (varsInitialized) {
        ...
        hillvallea::resource_allocation_scheme_process_pt process;
        process = std::make_shared<hillvallea::resource_allocation_scheme_process_t>(
                uhvFitnessFunction,
                localOptimizerIndexUHV_GOMEA,
                localOptimizerIndexUHV_ADAM,
                indexApplicationMethodUHV_ADAM,
                improvementMetricIndex,
