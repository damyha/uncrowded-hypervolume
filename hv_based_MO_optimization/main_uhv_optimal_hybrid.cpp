/**
 * UHV-OPTIMAL-HYBRID
 * This script applies all given local optimizers on a population in parallel for a given number of evaluations.
 * Then on all the obtained populations it reapplies the local optimizers (for the same number of evaluations)
 * to get a growing tree of populations
 * The branch that produces the most optimal solution in the end is written to a file.
 *
 *
...
#include "HillVallEA/OptimalHybrid/optimization_branch.h"
#include "HillVallEA/OptimalHybrid/optimal_hybrid_process.h"
...
bool write_statistics_per_branch;     // Write statistics for every population branch
bool write_solutions_per_branch;      // Write solutions for every population branch Todo:Not implemented yet
...
int reinitializeOptimizers;             // Let the optimizers keep their state/reinitialize optimizers every step
int linkageModelUHV_GOMEA;              // The linkage model of UHV-GOMEA
int modelUHV_ADAM;                      // The version of UHV-ADAM (ADAM/GAMO)
int versionUHV_ADAM;                    // The method of applying UHV-ADAM on a population
...
size_t numberOFUHVGOMEACallsPerStep;    // Number of UHV-GOMEA calls per step
size_t numberOfUHVADAMCallsPerStep;     // Number of UHV-ADAM calls per step
size_t maxMOEvaluationsPerGeneration;   // Maximum number of evaluations per generation
...
void printHelp()
{
    printf("Usage: uhv_optimal [-?] [-P] [-s] [-w] [-v] [-e] [-r] lmod vgrad mgrad reint pro dim ssize low upp pop eva sec vtr cea cgrad mogen alpha beta lmbda freq rnd wrp\n"); // [-n] [-f]
...
//    printf(" -w: Enable writing of solutions and their fitness every generation.\n");
    ...
    printf("reint: Let optimizers reinitialize every optimizatio step (0 = Keep optimizer state, 1 = Reinitialize.\n");
    ...
    printf("  cea: Number of EA calls per step.\n");
    printf("cgrad: Number of gradient algorithm calls per step.\n");
    printf("mogen: Number of MO-Evals per optimization generation.\n");
...
void printComputationSettings(const hillvallea::fitness_pt& single_objective_fitness_function)
{
    ...
    std::cout << "\tUHV-ADAM-application_method = " << versionUHV_ADAM << std::endl;
    std::cout << "\tReinitialize optimizers = " << (reinitializeOptimizers ? "yes": "no") << std::endl;
    std::cout << "\ttest_points = " << solution_set_size << std::endl;
    ...
    std::cout << "\tNumber of EA calls per step = " << numberOFUHVGOMEACallsPerStep << std::endl;
    std::cout << "\tNumber of gradient algorithm calls per step = " << numberOfUHVADAMCallsPerStep << std::endl;
    std::cout << "\tNumber of MO-Evaluations per optimization generation = " << maxMOEvaluationsPerGeneration << std::endl;
}

void printComputationResults(const hillvallea::optimal_hybrid_process_pt process)
{
//    std::cout << "Best:" << std::endl;
//    std::cout << "\tHV = " << std::fixed << std::setprecision(14) << -process->bestSolution.f << std::endl;
//    std::cout << "\tMO-fevals = " << process->uhv_optimizer->     h->active * solution_set_size << std::endl;
//    std::cout << "\truntime = " << double(clock() - opt->starting_time) / CLOCKS_PER_SEC << " sec" << std::endl;
//
//    std::cout << "pareto_front" << (use_bezier_interpolation ? number_of_reference_points : 0) << " = [";
//    for(auto & mo_test_sol : opt->best.mo_test_sols) {
...
void writeParametersFile()
{
    ...
    std::string text = "";
    text += "Algorithm:UHV-Optimal, ";
    text += "EAModel:" + std::to_string(linkageModelUHV_GOMEA) + ", ";
    text += "GradientModel:" + std::to_string(modelUHV_ADAM) + ", ";
    text += "GradientApplication:" + std::to_string(versionUHV_ADAM) + ", ";
    text += "GradientReinitialization:" + std::to_string(reinitializeOptimizers) +", ";
    ...
    text += "CallsPerStepEA:" + std::to_string(numberOFUHVGOMEACallsPerStep) + ", ";
    text += "CallsPerStepGradient:" + std::to_string(numberOfUHVADAMCallsPerStep) + ", ";
    text += "MOEvalsPerGeneration:" + std::to_string(maxMOEvaluationsPerGeneration) + ", ";
...
void initializeOptions(){
    ...
    // Write settings
    write_statistics_per_branch = false;
    write_solutions_per_branch = false;
        ...
        if (argv[*index][0] == '-')
        {
            ...
            else
            {
                ...
                switch (argv[*index][1])
                {
                    ...
                    case 's': write_statistics_per_branch = true; break;
//                    case 'w': write_solutions_per_branch = true; break;
...
void parseParameters(int argc, char **argv, const int *index)
{
    ...
    noError = noError && sscanf(argv[*index + 1], "%d", &modelUHV_ADAM);
    noError = noError && sscanf(argv[*index + 2], "%d", &versionUHV_ADAM);
    noError = noError && sscanf(argv[*index + 3], "%d", &reinitializeOptimizers);
    noError = noError && sscanf(argv[*index + 4], "%d", &problem_index);
    noError = noError && sscanf(argv[*index + 5], "%zd", &mo_number_of_parameters);
    noError = noError && sscanf(argv[*index + 6], "%zd", &solution_set_size);
    noError = noError && sscanf(argv[*index + 7], "%lf", &lower_init);
    noError = noError && sscanf(argv[*index + 8], "%lf", &upper_init);
    noError = noError && sscanf(argv[*index + 9], "%zd", &population_size);
    noError = noError && sscanf(argv[*index + 10], "%d", &maximum_number_of_mo_evaluations);
    noError = noError && sscanf(argv[*index + 11], "%d", &maximum_number_of_seconds);
    noError = noError && sscanf(argv[*index + 12], "%lf", &value_to_reach);
    noError = noError && sscanf(argv[*index + 13], "%zd", &numberOFUHVGOMEACallsPerStep);
    noError = noError && sscanf(argv[*index + 14], "%zd", &numberOfUHVADAMCallsPerStep);
    noError = noError && sscanf(argv[*index + 15], "%zd", &maxMOEvaluationsPerGeneration);
    noError = noError && sscanf(argv[*index + 16], "%lf", &f_alpha);        // Problem setting
    noError = noError && sscanf(argv[*index + 17], "%lf", &f_beta);         // Problem setting
    noError = noError && sscanf(argv[*index + 18], "%lf", &f_lambda);       // Problem setting
    noError = noError && sscanf(argv[*index + 19], "%lf", &f_frequency);    // Problem setting
    noError = noError && sscanf(argv[*index + 20], "%d", &random_seed);
    write_directory = argv[*index + 21];
...
void checkOptions()
{
    ...
    switch (linkageModelUHV_GOMEA) {
            case 0: localOptimizerIndexUHV_GOMEA = 64; break; // marginal linkage
            case 1: localOptimizerIndexUHV_GOMEA = 66; break; // linkage tree
            case 2: localOptimizerIndexUHV_GOMEA = 50; break; // full linkage
        default: {
            throw std::runtime_error(std::string("Error: GOMEA's linkage model is invalid (read: ") +
                std::to_string(localOptimizerIndexUHV_GOMEA) + ").\n");
        }
    }

    // Determine Gradient Algorithm local optimizer index
    switch (modelUHV_ADAM) {
        case 0: localOptimizerIndexUHV_ADAM = 50; break; // ADAM
        case 1: localOptimizerIndexUHV_ADAM = 64; break; // GAMO
        case 2: localOptimizerIndexUHV_ADAM = 80; break; // PLAIN GRADIENT
        case 3: localOptimizerIndexUHV_ADAM = 81; break; // LINE SEARCH APPROXIMATION
        default:{
            throw std::runtime_error(std::string("Error: The gradient algorithm's version is invalid (read: ") +
                                     std::to_string(modelUHV_ADAM) + ").\n");
        }
    }

    // Check Gradient Algorithm application method
    if (versionUHV_ADAM != 0 && versionUHV_ADAM != 1 && versionUHV_ADAM != 2 && versionUHV_ADAM != 3)
    {
        throw std::runtime_error("Unknown gradient algorithm application method detected: " +
            std::to_string(versionUHV_ADAM) + ".\n");
    }

    // Check that state is 0 or 1
    if (reinitializeOptimizers != 0 && reinitializeOptimizers != 1)
    {
        throw std::runtime_error(std::string("Error: state should be 0 or 1 not: ") +
            std::to_string(reinitializeOptimizers) + ").\n");
    }
    ...
    std::stringstream ss;
    ss << "_UHV-HYBRID" << linkageModelUHV_GOMEA << "_problem" << problem_index << "_p" << solution_set_size << "_run" << std::setw(3) << std::setfill('0') << random_seed;
...
int main(int argc, char **argv)
{
    ...
    if (vars_initialized) {
        ...
        if(!use_bezier_interpolation) {
            ...
            throw std::runtime_error("Unknown fitness function type.\n");
        }
        ...
        // Change sign of VTR due to minimalization of HV(X)
        value_to_reach *= -1;
        ...
        std::shared_ptr<hillvallea::optimal_hybrid_process_t> process;
        process = std::make_shared<hillvallea::optimal_hybrid_process_t>(
                fitness_function,
                localOptimizerIndexUHV_GOMEA,
                localOptimizerIndexUHV_ADAM,
                versionUHV_ADAM,
                reinitializeOptimizers,
                ...
                use_vtr,
                value_to_reach,
                numberOFUHVGOMEACallsPerStep,
                numberOfUHVADAMCallsPerStep,
                maxMOEvaluationsPerGeneration,
                random_seed,
                write_solutions_per_branch,
                write_statistics_per_branch,
                write_directory,
                file_appendix
        );
